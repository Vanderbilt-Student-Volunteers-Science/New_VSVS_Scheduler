...
def convert_to_military(time):
    ...
    :type time: str
    :raises ValueError if invalid time format
...
def calculate_free_time_start(class_start_time, school_travel_time):
    ...
    :return: first time a volunteer needs to be free to perform a lesson in military time
    """
...
def calculate_free_time_needed(class_start_time, class_end_time, school_travel_time):
    ...
    # FIXME: edge case on last time period
...
def military_to_free_time_array(day_of_week, free_time_start):
    ...
    # FIXME: The line below this comment doesn't do anything?
    min = int(min / 15) * 15
