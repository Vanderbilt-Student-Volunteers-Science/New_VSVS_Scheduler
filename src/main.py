...
def main():
    ...
    with open('../test/individuals.csv') as individuals_csv:  # opens individuals.csv as individuals_csv
        ...
        for row in csv_reader:
            volunteer_list.append(src.person.Person(row[1], row[2], row[3], row[4], row[8], row[9], row[10], row[12], row[13], row[16], row[18], row[20]))  # creates Person objects and adds them to volunteer_list, indices correspond to columns of responses in volunteers.csv
