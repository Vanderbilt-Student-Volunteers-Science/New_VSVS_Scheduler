...
# defining global constants before imports prevents circular dependency
MAX_TEAM_SIZE = 4
MIN_TEAM_SIZE = 4
...
def main():
    ...
    with open('../data/individuals.csv', 'r') as individuals_csv:  # opens individuals.csv as individuals_csv
        ...
        for row in csv_reader:
            ...
            # individuals.csv
            volunteer_list.append(src.volunteer.Volunteer(row[2], row[3], row[4], row[5], row[7], row[8], row[9],
    ...
    with open('../data/classrooms.csv', 'r') as classrooms_csv:  # opens classrooms.csv as classrooms_csv
        ...
        for row in csv_reader:
            ...
            classroom_list.append(src.classroom.Classroom(row[1], row[3], row[4], row[6], row[9], row[11], row[12], row[13]))
    ...
    src.assign.assign_drivers(src.assign.sort_by_availability(driver_list), classroom_list)
        ...
        for volunteer_id in range(1, len(volunteer_list)):
            csv_writer.writerow(
                [volunteer_id, volunteer_list[volunteer_id].group_number, volunteer_list[volunteer_id].first,
                 ...
                 volunteer_list[volunteer_id].phone, volunteer_list[volunteer_id].year_in_school,
                 '', volunteer_list[volunteer_id].major, '', volunteer_list[volunteer_id].assigned_driver,
                 volunteer_list[volunteer_id].assigned_t_leader])
